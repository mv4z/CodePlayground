...
object fp3 {
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    ...
    xs match {
      case Nil   => true
      case y::Nil => true
      case y::ys if(y == ys.head) => allEqual(ys)
      case _::ys => false
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    def RDAux [X] (x:X, xs:List[X],i:Int) :List[(Int,X)] = {
      xs match{
        case Nil => if (x==Nil) Nil else (i,x)::Nil
        case y::ys => if(y == x) RDAux(x,ys,i+1) else (i,x)::RDAux(y,ys,1)
      }
    }
      if(xs.length>0) RDAux(xs.head,xs.tail,1)
      else Nil
  }
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
    ...
    xs match{
      case Nil => true
      case y::ys if(!member(y, ys)) => allDistinct(ys)
      case _::ys => false
    }
  }
}

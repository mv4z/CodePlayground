...
object fp2 {
  ...
  def flatten [A] (xss:List[List[A]]) : List[A] = {
    // TODO: Provide definition here.
    //this code works correctly : for (xs <- xss; x <- xs) yield x 
    xss match{
      case Nil => Nil
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    ...
    xs match{
      case Nil => ""
      case y::ys => foldLeft(xs, "", (x:String, y:String) => x + y + term )
    }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    ...
    xs match{
      case Nil => ""
      case y::ys => foldRight(xs, "", (x:String, y:String) => x + delimiter + y)
    }
      ...
      case y::ys => 
        if (y >= a){
          0 
            ...
            1 + firstIndexNumGreaterThan(a, ys)
          }
    }
  }

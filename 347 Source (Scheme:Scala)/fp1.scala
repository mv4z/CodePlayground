...
object fp1 {
  ...
  val factTest : List[Int] = {
    ...
    val finList = (1 to 5).toList.map (n => fact(n))
    finList
  }
  ...
  def sumTailRecursive (xs : List[Int]) : Int = {
    ...
    xs match{
      case Nil => 0
      case y::ys => sumTailRecursiveAux( xs.head, xs.tail)
    }
  }
  ...
  def maxTailRecursiveAux (accumulator : Int, xs : List[Int]) : Int = {
    ...
    xs match{
      case Nil => accumulator
      case y::ys => accumulator max (maxTailRecursiveAux(xs.head, xs.tail))
